<script lang="ts">
  ...
  async function deleteExpense(expense: Expense) {
    ...
    try {
      ...
      await loadExpenses();
      showNotification(`${expense.source} expense deleted successfully!`, 'success');
    } catch (error) {
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
      ...
      <div class="flex gap-2">
        <button
          ...
          class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 disabled:bg-gray-400 transition-colors font-medium text-sm"
        >
          📊 Export CSV
        </button>
        <button
          ...
          class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 disabled:bg-gray-400 transition-colors font-medium"
        >
          + Add Expense
        </button>
      </div>
